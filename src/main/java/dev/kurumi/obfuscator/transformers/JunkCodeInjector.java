...
public class JunkCodeInjector implements Transformer {
    ...
    public void transform(ClassPool pool, ObfuscatorContext ctx) {
        ...
        for (ClassNode cn : pool.allClassNodes()) {
            ...
            // Skip framework-synthetic single-purpose helper classes
            // (e.g. {@code StringEncryption}'s stack probe, the
            // {@code AntiAgentTransformer} guard). They are tiny,
            // closed under their own logic, and have no real methods
            // to host predicate-wired honeypot calls -- adding decoys
            // to them shifts the honeypot/caller distribution so the
            // adversarial-name test has nothing left to wire.
            if (isFrameworkSyntheticHelper(cn)) continue;
    ...
    /**
     * A "framework-synthetic helper" is a class generated by another
     * transformer purely to host a single static utility method
     * (e.g. anti-agent guard, stack probe). Recognised structurally:
     * a final synthetic class whose simple name (last path segment)
     * starts with the per-JAR synthetic prefix, has no instance
     * fields, and exposes no more than two methods. We deliberately
     * do NOT match by name infix because the infix is owned by each
     * generating transformer and listing them here would couple
     * JunkCodeInjector to every helper's name choice.
     */
    private static boolean isFrameworkSyntheticHelper(ClassNode cn) {
        if ((cn.access & Opcodes.ACC_SYNTHETIC) == 0) return false;
        if ((cn.access & Opcodes.ACC_FINAL) == 0) return false;
        ...
        if (cn.methods == null) return true;
        if (cn.methods.size() > 3) return false;
        int simpleStart = cn.name.lastIndexOf('/') + 1;
        return simpleStart < cn.name.length() && cn.name.charAt(simpleStart) == '$';
    }
